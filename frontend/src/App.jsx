...
const PaymentPage = () => {
    const { navigate, currentApplication } = useApp();
    const [formData, setFormData] = useState({ applicationId: currentApplication?.id, amount: 200.00, paymentMethod: 'BKASH', transactionId: '' });
    const [loading, setLoading] = useState(false);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault(); setLoading(true);
        try { await MOCK_API.submitPayment(formData); alert('Payment information submitted successfully!'); navigate('dashboard'); }
        catch (error) { alert('Failed to submit payment information.'); }
        finally { setLoading(false); }
    };
    return (
        <AnimatedCard className="max-w-lg mx-auto">
            <h2 className="text-3xl font-bold text-[#4E2A2A] mb-4">Complete Your Payment</h2>
            <div className="bg-[#E97451]/10 p-4 rounded-xl mb-6 text-[#c15c41] border border-[#E97451]/20"><p><strong>Service:</strong> {getServiceName(currentApplication.serviceId)}</p><p><strong>Application ID:</strong> {currentApplication.id}</p><p className="font-bold text-xl mt-2"><strong>Amount to Pay:</strong> BDT {formData.amount.toFixed(2)}</p></div>
